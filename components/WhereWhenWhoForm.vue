<template>
  <v-container>
    <text-field :label="'どこで？'" v-model="internalWhereValue" />
    <date-picker :label="'いつ？'" v-model="internalWhenValue" />
    <text-field :label="'だれが？'" v-model="internalWhoValue"/>
  </v-container>
</template>
...
export default class WhereWhenWhoForm extends Vue {
  ...
  set internalWhereValue(v: String) {
    if (this.value.where !== v) {
      this.$emit('input', {where: v, when: this.value.when, who: this.value.who})
    }
  }
  ...
  set internalWhenValue(v: String) {
    if (this.value.when !== v) {
      this.$emit('input', {where: this.value.where, when: v, who: this.value.who})
    }
  }
  ...
  set internalWhoValue(v: String) {
    if (this.value.who !== v) {
      this.$emit('input', {where: this.value.where, when: this.value.when, who: v})
    }
  }
}
</script>
